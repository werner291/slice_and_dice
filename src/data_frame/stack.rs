    ...
    D::Output: Clone + Default,
{
    ...
    > {
        ...
        let outer_index = NumericRangeIndex::new(0, dfs.len());
        let compound_index = CompoundIndex {
            indices: (outer_index, union_index.clone()),
        };
...
mod tests {
    ...
    fn test_stack_sparse_matching() {
        ...
        assert_eq!(stacked.index.indices.0.size(), 2); // Outer index size
        assert_eq!(stacked.index.indices.1.indices, vec![1, 3, 5]); // Inner index
    ...
    fn test_stack_sparse_mismatching() {
        ...
        assert_eq!(stacked.index.indices.0.size(), 2); // Outer index size
        assert_eq!(stacked.index.indices.1.indices, vec![1, 2, 3, 5, 6]); // Inner index
    ...
    fn test_stack_sparse_interpolation_methods() {
        ...
        assert_eq!(stacked_prev.index.indices.1.indices, vec![1, 3, 5]); // Inner index
        ...
        assert_eq!(stacked_next.index.indices.1.indices, vec![1, 3, 5]); // Inner index
        ...
        assert_eq!(stacked_default.index.indices.1.indices, vec![1, 3, 5]); // Inner index
    ...
    fn test_stack_sparse_large_matching() {
        ...
        assert_eq!(stacked.index.indices.0.size(), 2); // Outer index size
        assert_eq!(stacked.index.indices.1.indices.len(), 12); // Inner index size
        assert_eq!(
            stacked.index.indices.1.indices,
            vec![1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]
        ); // Inner index
    ...
    fn test_stack_sparse_large_mismatching() {
        ...
        assert_eq!(stacked.index.indices.0.size(), 2); // Outer index size
        assert_eq!(stacked.index.indices.1.indices.len(), 24); // Inner index size (union of both indices)

        // Union of indices should be [0, 1, 2, 3, ..., 22, 23]
        let expected_indices: Vec<i32> = (0..24).collect();
        assert_eq!(stacked.index.indices.1.indices, expected_indices);
    ...
    fn test_stack_sparse_large_interpolation() {
        ...
        assert_eq!(
            stacked_next.index.indices.1.indices,
            stacked_prev.index.indices.1.indices
        );
        ...
        assert_eq!(
            stacked_default.index.indices.1.indices,
            stacked_prev.index.indices.1.indices
        );
